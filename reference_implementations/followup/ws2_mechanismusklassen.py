# -*- coding: utf-8 -*-
"""
ws2_mechanismusklassen.py -- WS2: stratification of the skeletal-dysplasia
disease genes by seven external mechanism classes (GO sets from
`ws2_go_klassen.R`), tested against the FIXED 173/147-gene program.
...
matched draw mechanism from `reference_implementations/52_human_genetics_anchor.py`, only with
...
ALT_ANALYSE = WURZEL / "reference_implementations"
NEU = WURZEL / "Neu"
...
# 52_human_genetics_anchor.py loaded as a module (importable without hyphen
# despite the leading digit in the filename), in order to reuse `raster`,
# `gematcht`, `exonlaengen`, `sym2ens` UNCHANGED.
_spec = _ilu.spec_from_file_location("anker52", ALT_ANALYSE / "52_human_genetics_anchor.py")
...
def main():
    ...
    HG = anker52.raster(K)
    log("background (draw mechanism 52_human_genetics_anchor.py): %d genes" % len(HG))
    ...
    # ---------------------------------------------------------- positive control
    log("\n" + "-" * 78)
    log("POSITIVE CONTROL (reproduced from 52_human_genetics_anchor.py, unchanged)")
    eich = []
