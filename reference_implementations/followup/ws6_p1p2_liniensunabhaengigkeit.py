# -*- coding: utf-8 -*-
"""
ws6_p1p2_liniensunabhaengigkeit.py -- WS6, checks P1 and P2.
...
Data source (NOT recomputed, verbatim from the frozen tables):
  * per dataset `dWT` per gene  -> 20_Exploration/derived_data/20d_gene_XX.csv
    (the same source that 56_calibration_eighteen.py uses for lineage calibration)
  * the lineage calibration per dataset -> derived_data/M_kalibrierung/eichung_achtzehn.csv
  * the module (173 genes, direction ri) -> reference_implementations/_module.py (MODUL)
  * the statistic -> reference_implementations/_module.py `konkordanz()` (sign test against
...
def gencode_karte() -> dict:
    """The same Ensembl->symbol map as in 56_calibration_eighteen.py."""
    import gzip
    import re
