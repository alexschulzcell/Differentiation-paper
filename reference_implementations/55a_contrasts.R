# =============================================================================
# 55a_contrasts.R -- phase M-E, test 1: single-sample contrasts per dataset.
# =============================================================================
...
# loader `13_load18.R`, computation runs on its `Z` (per-gene z per dataset)
# with the same algebra as `kern()` in `03_metric.R`.
#
# Only the SIGN (-1/0/+1) is written, because the convergence rule of the
# main part is sign-based. Counting happens in `55b_analysis.py`.
#
...
METHODEN <- Sys.getenv("PAPER_V2_METHODEN",
  file.path(WURZEL, "reference_implementations", "manuscript", "methods"))

args   <- commandArgs(trailingOnly = TRUE)
NUR_PUNKTE <- if (length(args) >= 1) as.integer(strsplit(args[1], ",")[[1]]) else 1:18

source(file.path(METHODEN, "13_load18.R"))
...
cat(sprintf("\n55a_contrasts.R -- %d Datensaetze, B = %d, Seed %d\n",
