# -*- coding: utf-8 -*-
"""
31_donor_cells_build_calibrate.py -- phase M-D: build the cells, check for LiCl, calibrate.
...
Per cell are formed (§3 of the preregistration, identical to `12_metric_reference.R`,
function `kern`):
...
from `13_geo_matrices_to_metric_format.R`: no time point is selected.
...
def lade_GSE218101():
    ...
    isogenic 2x2 -- they are not used (as in `13_geo_matrices_to_metric_format.R`)."""
...
def lade_LAMA5_USC():
    ...
    **one** donor. The series is an isogenic lesion series, not a donor
    ...
    quantity that `12_metric_reference.R` also computes.
    """
