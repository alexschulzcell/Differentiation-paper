# =============================================================================
# 11_load_18_datasets.R -- Loads all EIGHTEEN points: the eleven from 10_load_reference_metric.R and the
#                seven from step B, in exactly the same format.
# =============================================================================
# As in 10_load_reference_metric.R: do not rewrite it, call it. This script does not implement
# the metric -- it appends the seven new data sets to DATEN in the same
# preparation that vorb2() in 12_metric_reference.R applies to the eleven old ones (rlog
...
# 10_load_reference_metric.R is sourced from the repaired copy IN THE REPOSITORY (identical in
...
source(file.path(METHODEN, "10_load_reference_metric.R"))
stopifnot(length(DATEN) == 11)

source(file.path(SESSIONS, "03_Metrik_Elf_Punkte", "reference_implementations",
       "14_geo_matrices_s5_format.R"))

# Word for word from 12_metric_reference.R (zmat / vorb2), only without the loading part
...
cat(sprintf("11_load_18_datasets.R: %d data sets in memory, points %s.\n",
