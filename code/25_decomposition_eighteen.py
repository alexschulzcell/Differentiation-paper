# -*- coding: utf-8 -*-
"""
25_decomposition_eighteen.py -- the three-way decomposition on all 18 data sets.
...
`reference_implementations/_module.py`, the marker sets from
`reference_implementations/_marker.py`, `dWT` from the frozen `20d_gene_*.csv`,
...
worked through in `code/29_calibration_gene_space.py`.
`reference_implementations/56_calibration_eighteen.py` is corrected
...
def karte_gencode() -> dict:
    ...
    56_calibration_eighteen.py -- imported, not rebuilt. The file name starts
    with a digit, hence importlib."""
    p = WURZEL / "reference_implementations" / "56_calibration_eighteen.py"
