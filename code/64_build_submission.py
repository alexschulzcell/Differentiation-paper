"""
64_build_submission.py -- assemble the submission package for iScience
...
Purpose   Rebuilds submission/ from scratch. File names and structure follow
          the iScience Final File Requirements:
            ...
            Cover letter.docx
            KRT.docx                    (from code/63_key_resources_table.py)
...
def number_references(md: str, superscript: bool = True) -> str:
    ...
    # cross-check (71_check_references.py) reports them separately.
    for n, _ in refs:
        if n not in order:
            number_for(n)
...
def build_figures() -> None:
    ...
    # code/62_graphical_abstract.py draws it at 300 rather than 600 dpi. Here
    ...
    if ga.size != (1200, 1200):
        raise RuntimeError(
            ...
            "Re-run code/62_graphical_abstract.py.")
...
Written by `code/64_build_submission.py`. Everything in `submission/` is built
...
- [x] **No preprint.**
- [x] `python code/70_check_numbers.py` green
- [x] `python code/71_check_references.py` green
- [x] `python code/72_check_language.py` green
...
def main() -> int:
    ...
    print(f"64_build_submission.py -- package into {OUT} (style: {a.style})")
    tpl = templates()
    ...
    if not (OUT / "KRT.docx").exists():
        print("  ! KRT.docx missing -- run code/63_key_resources_table.py first")
