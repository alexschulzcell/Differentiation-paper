# =============================================================================
# 24_gene_sets_v2.R -- put Figure 2D on gene sets that can carry it.
#
# The gene sets are fixed and stored BEFORE this run
# (`code/24a_gene_sets_v2_build.R`), each with its source, version and
...
# (`reference_implementations/_enrichment.R`), the same one that produced the
# narrow run.
#
# -----------------------------------------------------------------------------
...
source(file.path(WURZEL, "reference_implementations", "_enrichment.R"))
...
# by `code/50_panel_data.py` from `results/gensaetze_v2_anreicherung.csv`, not
