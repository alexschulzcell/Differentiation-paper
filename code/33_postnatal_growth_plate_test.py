# -*- coding: utf-8 -*-
"""
33_postnatal_growth_plate_test.py -- the postnatal anchor: module trend and
...
Inputs   results/gse288028_pseudobulk.csv.gz (from
         32_postnatal_growth_plate_annotation.py; one row per (zone, donor)
...
detection limit, the same test from code/20_in_vivo_donor_test.py also runs on
...
reference_implementations/_marker.py). On a collision the CALIBRATION set is
...
reference_implementations/_module.py, the trend test imported from
code/20_in_vivo_donor_test.py.
"""
...
RES = WURZEL / "results"
SATZ_DATEI = WURZEL / "data_raw" / "_referenz" / "wachstumsfuge_zonen" / "saetze.csv"
...
# code/32_postnatal_growth_plate_annotation.py (POSTNATALE_PROBEN=alle).
