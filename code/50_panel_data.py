# -*- coding: utf-8 -*-
"""
50_panel_data.py -- one CSV per PANEL, not per figure.
...
Rule     NO second implementation of any metric. This script recomputes
         ...
         (`reference_implementations/_module.py`,
         `reference_implementations/manuscript/methods/03_metric.R`) and
...
def f1() -> None:
    ...
    # space, see code/29_calibration_gene_space.py)
...
def f2() -> None:
    ...
    # `code/24_gene_sets_v2.R`; the sets were fixed before the run. The narrow
    ...
    # code/25_decomposition_eighteen.py. The primary run is the FULL gene
...
def f6() -> None:
    ...
    # Figure F6 draws a forest and range plot (60_figures_main.R::bau_f6)
    ...
    # the same levels is in 70_check_numbers.py.
...
def main() -> None:
    log("=" * 78)
    log("50_panel_data.py -- one CSV per panel to figures/data/")
    log("=" * 78)
