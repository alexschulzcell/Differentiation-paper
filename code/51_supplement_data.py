# -*- coding: utf-8 -*-
"""
51_supplement_data.py -- panel data for the supplementary figures and for the
                         supplementary tables.

Purpose  Adds to `50_panel_data.py` the parts that are only needed in the
...
Rule     As in 50_panel_data.py: no second implementation of any metric.
...
def tabellen() -> None:
    ...
    # code/27_in_vivo_gene_decomposition.py.
    ...
    # TS12 -- the primary publications of every GEO series used, pulled from
    # GEO (code/28_geo_primary_publications.py).
    # TS13 -- does the fetal in vivo trend hang on the single hypertrophic
    # point? The decision rule is in the header of
    # code/34_hypertrophic_zone_sensitivity.py.
    ...
    # (28_geo_primary_publications.py) and the four series whose publication
    ...
    pa = pd.read_csv(WURZEL / "data_raw" / "_referenz" /
                     "geo_primaerarbeiten.csv")
    hand = pd.read_csv(WURZEL / "data_raw" / "_referenz" /
                       "geo_primaerarbeiten_handbeleg.csv")
...
def main() -> None:
    log("=" * 78)
    log("51_supplement_data.py -- supplement panels and tables")
    log("=" * 78)
